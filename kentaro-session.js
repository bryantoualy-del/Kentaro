...
const KEY='kentaro-session-v3',LEGACY_V2='kentaro-session-v2',LEGACY_SESSION='kentaro-session-v1',LEGACY_SOCIAL='kentaro-social-v2';
...
const desk=document.createElement('article');desk.className='session-desk';desk.innerHTML=`
 ...
 <section class="people-card"><header><div><span>PNJ importants de la session</span><h3>Fiches à transmettre à Obsidian</h3><p>Les figurants restent de simples mentions. Crée une fiche uniquement lorsqu’un PNJ mérite d’être suivi.</p></div><div><button id="addPerson" class="btn-gold">＋ Créer une fiche PNJ</button></div></header><div id="peopleList" class="people-list"></div></section>
...
async function renderPeople(){
 ...
 q('#peopleList').innerHTML=people.length?people.map(x=>`<article class="person-card" data-person-card="${x.id}"><div class="person-avatar" data-person-image="${x.id}">${esc((x.name||'?').slice(0,1).toUpperCase())}</div><div><b>${esc(x.name||'Sans nom')}</b><small>${esc(x.category||'Inconnu')}${x.status?` · ${esc(x.status)}`:''}</small><p>${esc(x.note||'Aucune note')}</p><span>Fiche PNJ${x.promise?' · Promesse':''}${x.debt?' · Dette':''}${x.goal?' · Objectif':''}</span></div><footer><button data-edit-person="${x.id}">Modifier la fiche</button></footer></article>`).join(''):'<div class="session-empty">Aucune fiche PNJ. Les noms secondaires peuvent rester dans les notes.</div>';
...
function addOrUpdateEntry(){const text=entryText.value.trim();if(!text)return;const type=entryType.value,personId=type==='PNJ'?entryPerson.value:'',pnjKind=type==='PNJ'?entryPnjKind.value:'';const previous=editingEntry>=0?data.active.entries[editingEntry]:null,item={id:previous?.id||uid(),type,text,createdAt:previous?.createdAt||new Date().toISOString(),personId,pnjKind};if(editingEntry>=0){data.active.entries[editingEntry]=item;editingEntry=-1;q('#addEntry').textContent='Ajouter';q('#cancelEntryEdit').hidden=true}else data.active.entries.push(item);entryText.value='';persist();renderEntries();renderPeople()}
...
function personForm(x){return`<label>Nom<input name="name" required value="${esc(x.name||'')}" placeholder="Nom du PNJ"></label><div class="dialog-grid"><label>Catégorie<select name="category">${PERSON_CATEGORIES.map(c=>`<option ${c===x.category?'selected':''}>${c}</option>`).join('')}</select></label><label>Statut<input name="status" value="${esc(x.status||'')}" placeholder="Actif, disparu, hostile…"></label></div><label>Ce que Kentaro sait<textarea name="note">${esc(x.note||'')}</textarea></label><div class="dialog-grid"><label>Dette<input name="debt" value="${esc(x.debt||'')}"></label><label>Promesse<input name="promise" value="${esc(x.promise||'')}"></label></div><label>Objectif / prochaine étape<input name="goal" value="${esc(x.goal||'')}"></label><label class="portrait-field">Portrait<input name="portrait" type="file" accept="image/png,image/jpeg,image/webp"><small>${x.assetId?'Un portrait est déjà associé. Choisir un fichier le remplacera.':'PNG, JPEG ou WebP. Il sera transmis à Obsidian avec la fiche.'}</small></label><div class="dialog-actions"><button value="save" class="btn-gold">${x.id?'Enregistrer':'Créer la fiche'}</button>${x.id?'<button type="button" data-delete-person class="utility-danger">Supprimer</button>':''}</div>`}
function editPerson(id,seed={}){
 const existing=data.active.people.find(x=>x.id===id),person=existing||{id:'',name:'',category:'Inconnu',status:'',note:seed.note||'',debt:'',promise:'',goal:''},dlg=q('#personEditor'),form=$('form',dlg);q('#personEditorTitle').textContent=existing?'Modifier la fiche PNJ':'Créer une fiche PNJ';q('#personEditorBody').innerHTML=personForm(person);dlg.showModal();
 form.onsubmit=async e=>{if(e.submitter?.value!=='save')return;e.preventDefault();const fd=new FormData(form),name=String(fd.get('name')||'').trim();if(!name)return;const target=existing||{id:uid()};['name','category','status','note','debt','promise','goal'].forEach(k=>target[k]=String(fd.get(k)||'').trim());const portrait=fd.get('portrait');if(portrait instanceof File&&portrait.size){if(portrait.size>12*1024*1024)return alert('Cette image dépasse 12 Mo. Choisis une version plus légère.');try{const media=await mediaPut(portrait,target.assetId||uid());target.assetId=media.id;target.imageType=media.type;target.imageOriginalName=media.originalName}catch(error){return alert(`Impossible d’enregistrer cette image : ${error.message}`)}}target.updatedAt=new Date().toISOString();if(!existing)data.active.people.push(target);if(Number.isInteger(seed.sourceEntryIndex)&&data.active.entries[seed.sourceEntryIndex]){data.active.entries[seed.sourceEntryIndex].personId=target.id;data.active.entries[seed.sourceEntryIndex].pnjKind='information'}persist();dlg.close();await renderPeople();renderEntries();renderPnjCapture(target.id);api.log(`♜ Fiche PNJ préparée : ${target.name}.`);notify(`${target.name} sera transmis à Obsidian ✓`)};
...
q('#peopleList').onclick=e=>{const edit=e.target.closest('[data-edit-person]');if(edit)editPerson(edit.dataset.editPerson)};
...
async function buildObsidianPackage(session=data.active){
 ...
 for(const person of people){
  addText(`02 - Personnages/PNJ/${safeName(person.name)}.md`,personMarkdown(person,session),'person','skip-existing');
 ...
 const exportedAt=new Date(),stamp=exportedAt.toISOString().replace(/[-:]/g,'').slice(0,13),receipt=`# Import Kentaro — ${session.date||isoDate()}\n\n- **Session :** [[${base}]]\n- **Identifiant :** \`${session.id||'session-sans-id'}\`\n- **Fiches PNJ créées :** ${people.length}\n- **Portraits :** ${people.filter(x=>x.assetId).length}\n- **Export réalisé :** ${exportedAt.toLocaleString('fr-FR')}\n\n> Les simples mentions restent dans la session. Seules les fiches créées explicitement sont ajoutées au registre PNJ.\n`;
 addText(`98 - Archives/Imports Kentaro/${base} - Import ${stamp}.md`,receipt,'receipt','rename');
 const manifest={schema:'kentaro.obsidian-import',version:4,createdAt:new Date().toISOString(),vaultHint:'DND',importer:{id:'kentaro-session-importer',minimumVersion:'1.2.0'},session:{id:session.id||'',date:session.date||isoDate(),title:session.title||'Session de Kentaro',note:base},routes,personUpdates};

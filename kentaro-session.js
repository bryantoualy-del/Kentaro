...
const desk=document.createElement('article');desk.className='session-desk';desk.innerHTML=`
 ...
 <section class="session-export"><div><span>Fin de séance</span><b>Import Obsidian guidé</b><small>Sur iPad : partage le paquet au raccourci « Importer session Kentaro ». Le ZIP manuel reste disponible.</small></div><div><button id="previewMarkdown">Aperçu</button><button id="setupObsidian">Configurer l’iPad</button><button id="shareObsidian" class="btn-gold">Partager vers Obsidian</button><button id="exportObsidian">Télécharger ZIP</button><button id="closeSession" class="session-close">Clore la session</button></div></section>
 ...
 <dialog id="obsidianSetup" class="session-dialog obsidian-setup"><form method="dialog"><header><div><span class="session-kicker">Installation unique</span><h3>Raccourci « Importer session Kentaro »</h3></div><button value="cancel" aria-label="Fermer">×</button></header><ol><li>Dans Raccourcis, crée un raccourci visible dans la feuille de partage et acceptant les fichiers ZIP.</li><li>Ajoute <b>Extraire l’archive</b> depuis l’entrée du raccourci.</li><li>Dans le dossier extrait, distribue le contenu des quatre dossiers placés sous <code>Contenu</code> vers leurs dossiers homonymes du coffre <code>DND</code>.</li><li>Pour les sessions, demande confirmation si le fichier existe. Pour les PNJ, conserve toujours la fiche existante. Les portraits Kentaro peuvent être remplacés.</li><li>Termine par une notification « Session Kentaro importée » puis ouvre Obsidian.</li></ol><p>Le fichier <code>_kentaro-import.json</code> inclus dans chaque export décrit également toutes les destinations et règles de conflit.</p><button value="cancel" class="btn-gold">J’ai compris</button></form></dialog>
...
async function buildObsidianPackage(session=data.active){
 const people=touchedPeople(session),base=`${session.date||isoDate()} — ${safeName(session.title)}`,payload='Contenu',files=[],routes=[];
 ...
 const manifest={schema:'kentaro.obsidian-import',version:1,createdAt:new Date().toISOString(),vaultHint:'DND',session:{id:session.id||'',date:session.date||isoDate(),title:session.title||'Session de Kentaro',note:base},routes};
 files.unshift({path:'_kentaro-import.json',text:JSON.stringify(manifest,null,2)},{path:'LIRE AVANT IMPORT.md',text:`# Import automatique Kentaro\n\nSur iPad, partage directement le ZIP au raccourci **Importer session Kentaro**.\n\nLe raccourci lit \`_kentaro-import.json\` et distribue les fichiers dans le coffre **DND**.\n\nEn secours, ouvre le dossier \`${payload}\` et copie individuellement son contenu dans les dossiers correspondants du coffre. Ne remplace jamais un dossier complet.\n`});
...
q('#shareObsidian').onclick=async e=>{e.currentTarget.disabled=true;try{await exportObsidian(data.active,'share')}catch(error){alert(`Partage impossible : ${error.message}`)}finally{e.currentTarget.disabled=false}};

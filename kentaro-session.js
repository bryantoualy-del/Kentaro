...
const desk=document.createElement('article');desk.className='session-desk';desk.innerHTML=`
 ...
 <section class="people-card"><header><div><span>Personnes rencontrées</span><h3>Relations et portraits</h3><p>Les nouvelles personnes peuvent devenir des fiches PNJ ; une fiche Obsidian existante ne sera jamais écrasée automatiquement.</p></div><div><button id="peopleScope">Voir toutes</button><button id="addPerson" class="btn-gold">＋ Personne</button></div></header><div id="peopleList" class="people-list"></div></section>
 <section class="session-export"><div><span>Fin de séance</span><b>Import Obsidian en un geste</b><small>Télécharge le paquet, puis laisse le plugin Kentaro le classer directement dans ton coffre.</small></div><div><button id="previewMarkdown">Aperçu</button><button id="setupObsidian">Installer le plugin</button><button id="exportObsidian" class="btn-gold">1 · Télécharger</button><button id="openObsidian">2 · Ouvrir Obsidian</button><button id="closeSession" class="session-close">Clore la session</button></div></section>
 ...
 <dialog id="obsidianSetup" class="session-dialog obsidian-setup"><form method="dialog"><header><div><span class="session-kicker">Installation unique</span><h3>Plugin « Kentaro Session Importer »</h3></div><button value="cancel" aria-label="Fermer">×</button></header><ol><li>Dans Obsidian, installe puis active le module communautaire <b>BRAT</b>.</li><li>Dans BRAT, choisis <b>Add beta plugin</b> et saisis <code>bryantoualy-del/Kentaro</code>.</li><li>Sélectionne la dernière version puis active <b>Kentaro Session Importer</b> dans les modules installés.</li><li>À chaque fin de séance : télécharge le ZIP, ouvre Obsidian, touche l’icône lune et sélectionne le fichier.</li><li>Vérifie l’aperçu puis touche <b>Importer dans ce coffre</b>. La note de session s’ouvrira automatiquement.</li></ol><p>Le plugin travaille entièrement en local. Il ne remplace jamais une fiche PNJ existante et crée une copie si une session de même nom existe déjà.</p><button value="cancel" class="btn-gold">J’ai compris</button></form></dialog>
...
function renderPnjCapture(personId='',kind='information'){const active=entryType.value==='PNJ',selected=personId||entryPerson.value;pnjContext.hidden=!active;entryPerson.innerHTML=`<option value="">${data.people.length?'Choisir un PNJ…':'Ajoute d’abord une personne'}</option>${data.people.map(x=>`<option value="${esc(x.id)}">${esc(x.name||'Sans nom')}</option>`).join('')}`;if(selected&&data.people.some(x=>x.id===selected))entryPerson.value=selected;entryPnjKind.value=PNJ_KINDS[kind]?kind:'information';entryText.placeholder=active?'Information à rattacher à cette personne…':'Que vient-il de se passer ?'}
...
function addOrUpdateEntry(){const text=entryText.value.trim();if(!text)return;const type=entryType.value,personId=type==='PNJ'?entryPerson.value:'',pnjKind=type==='PNJ'?entryPnjKind.value:'';if(type==='PNJ'&&!personId){notify('Choisis d’abord la personne concernée');entryPerson.focus();return}const previous=editingEntry>=0?data.active.entries[editingEntry]:null,item={id:previous?.id||uid(),type,text,createdAt:previous?.createdAt||new Date().toISOString(),personId,pnjKind};if(editingEntry>=0){data.active.entries[editingEntry]=item;editingEntry=-1;q('#addEntry').textContent='Ajouter';q('#cancelEntryEdit').hidden=true}else data.active.entries.push(item);if(personId&&!data.active.peopleTouched.includes(personId))data.active.peopleTouched.push(personId);entryText.value='';persist();renderEntries();renderPeople()}
...
q('#entryList').onclick=e=>{const edit=e.target.closest('[data-edit-entry]'),del=e.target.closest('[data-delete-entry]');if(edit){editingEntry=Number(edit.dataset.editEntry);const item=data.active.entries[editingEntry];entryType.value=item.type;renderPnjCapture(item.personId||'',item.pnjKind||'information');entryText.value=item.text;entryText.focus();q('#addEntry').textContent='Mettre à jour';q('#cancelEntryEdit').hidden=false}if(del){data.active.entries.splice(Number(del.dataset.deleteEntry),1);persist();renderEntries()}};
...
function imageFileName(person){const token=String(person.assetId||person.id||'kentaro').replace(/[^a-z0-9]/gi,'').slice(0,8)||'kentaro';return`PNJ - ${safeName(person.name)} - ${token}.${imageExtension(person)}`}
...
async function buildObsidianPackage(session=data.active){
 ...
 addText(`01 - Sessions/${base}.md`,sessionMarkdown(session),'session','ask');
 for(const person of people){
  if(person.exportCard!==false)addText(`02 - Personnages/PNJ/${safeName(person.name)}.md`,personMarkdown(person,session),'person','skip-existing');
 ...
 const exportedAt=new Date(),stamp=exportedAt.toISOString().replace(/[-:]/g,'').slice(0,13),receipt=`# Import Kentaro — ${session.date||isoDate()}\n\n- **Session :** [[${base}]]\n- **Identifiant :** \`${session.id||'session-sans-id'}\`\n- **Fiches PNJ proposées :** ${people.filter(x=>x.exportCard!==false).length}\n- **Portraits :** ${people.filter(x=>x.assetId).length}\n- **Export réalisé :** ${exportedAt.toLocaleString('fr-FR')}\n\n> Ce reçu peut être conservé. L'importeur ne remplace jamais automatiquement une fiche PNJ déjà présente.\n`;
 addText(`98 - Archives/Imports Kentaro/${base} - Import ${stamp}.md`,receipt,'receipt','rename');
 const manifest={schema:'kentaro.obsidian-import',version:3,createdAt:new Date().toISOString(),vaultHint:'DND',importer:{id:'kentaro-session-importer',minimumVersion:'1.1.0'},session:{id:session.id||'',date:session.date||isoDate(),title:session.title||'Session de Kentaro',note:base},routes,personUpdates};
 files.unshift({path:'_kentaro-import.json',text:JSON.stringify(manifest,null,2)},{path:'LIRE AVANT IMPORT.md',text:`# Import Kentaro pour Obsidian\n\nDans Obsidian, lance la commande **Importer une session Kentaro** ou touche l’icône lune.\n\nSélectionne ce ZIP, vérifie l’aperçu puis confirme. Le plugin classe automatiquement la session, les fiches PNJ, les portraits et le reçu d’import dans le coffre actif.\n\nLe traitement est local et aucune fiche PNJ existante n’est écrasée.\n`});
 return{blob:await makeZip(files),name:`Kentaro - ${base}.zip`,count:routes.length,manifest}
}
...
window.KentaroSession={exportObsidian,buildObsidianPackage,getData:()=>clone(data),selectView:selectJournalView};

...
const titleIcon=el=>{const t=el.textContent.toLowerCase();if(t.includes('armure d’agathys'))return'agathys';if(t.includes('maléfice'))return'hex';if(t.includes('foulée brumeuse'))return'misty';if(t.includes('invisibilité'))return'invisible';if(t.includes('voile spirituel'))return'shroud';if(t.includes('porte dimensionnelle'))return'door';if(t.includes('ombre d’égarement'))return'shadow';if(t.includes('bannissement'))return'banish';if(t.includes('perturbations synaptiques'))return'synaptic';if(t.includes('décharge occulte'))return'eldritch';if(t.includes('solinar'))return'bladeSun';if(t.includes('sélhane'))return'bladeMoon';if(t.includes('cible'))return'target';if(t.includes('spectre'))return'spectre';if(t.includes('journal'))return'journal';if(t.includes('attaque'))return'action';if(t.includes('déf')||t.includes('armure')||t.includes('tombeau'))return'defense';if(t.includes('sort'))return'spells';if(t.includes('relation')||t.includes('mémoire'))return'social';return null};
let queued=false;
function decorate(){queued=false;document.querySelectorAll('.tab[data-tab]').forEach(el=>put(el,{combat:'combat',spells:'spells',defense:'defense',items:'items',spectre:'spectre',social:'social',journal:'journal'}[el.dataset.tab]));[['#ecoAction b','action'],['#ecoBonus b','bonus'],['#ecoReaction b','reaction'],['#ecoMove b','move'],['#ecoConc b','conc'],['.turn-next','turn'],['.turn-rest:not(.long)','rest'],['.turn-rest.long','shroud'],['#curseToggle','curse'],['#shroudToggle','shroud']].forEach(([s,n])=>document.querySelectorAll(s).forEach(el=>put(el,n)));document.querySelectorAll('.action-title').forEach(el=>{const n=titleIcon(el);if(n){put(el,n);if(['agathys','hex','misty','invisible','shroud','door','shadow','banish','synaptic','eldritch'].includes(n))el.classList.add('spell-title','spell-title-'+n)}})}

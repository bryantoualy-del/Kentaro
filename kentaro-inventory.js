...
const getIconChoice=key=>iconChoices.find(x=>x.key===key)||iconChoices[0];
...
function renderIconPicker(){
 ...
 picker.innerHTML=iconChoices.map(icon=>`<button type="button" class="inventory-icon-choice${icon.key===selected?' selected':''}" data-icon-key="${esc(icon.key)}" role="radio" aria-checked="${icon.key===selected}">${icon.icon?`<img src="${esc(icon.icon)}" alt="">`:`<span class="icon-glyph">${esc(icon.glyph)}</span>`}<small>${esc(icon.label)}</small></button>`).join('');
...
function cleanImportedInventory(raw){
 ...
 clean.custom=source.custom.slice(0,200).map((item,index)=>({id:`import-${Date.now().toString(36)}-${index}`,name:String(item?.name||'').trim().slice(0,80),category:categories[item?.category]?item.category:'misc',qty:Math.max(0,Math.min(99,Number(item?.qty)||0)),notes:String(item?.notes||'').slice(0,500),iconKey:getIconChoice(item?.iconKey).key,description:String(item?.description||'Objet importé dans le registre de Kentaro.').slice(0,300)})).filter(item=>item.name);
 return clean;
}
